...
def seed_path(base_dir: Path = BASE_DIR) -> Path:
    return base_dir / SEED_FILENAME


def legacy_seed_path(base_dir: Path = BASE_DIR) -> Path:
    return base_dir / LEGACY_SEED_FILENAME


def seed_contract_error(base_dir: Path = BASE_DIR) -> str:
    path = seed_path(base_dir)
    legacy = legacy_seed_path(base_dir)
    if legacy.exists():
        return (
            f"required file not found: {path}. "
            "seed.txt is no longer read automatically; copy or rename it to seed.md when you are ready to migrate."
        )
